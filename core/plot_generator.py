...
class PlotGenerator:
    ...
    def generate_histogram(
        ...
        if df is None or df.empty or column not in df.columns:
            print(
                f"PlotGenerator Error: Columna '{column}' no encontrada o DataFrame vacío para histograma."
            )
            return None
        if not pd.api.types.is_numeric_dtype(df[column]):
            print(
                f"PlotGenerator Error: La columna '{column}' no es numérica para histograma."
            )
            return None
    ...
    def generate_scatterplot(
        ...
        if (
            ...
            print(
                f"PlotGenerator Error: Columnas '{x_column}' o '{y_column}' no encontradas o DataFrame vacío para dispersión."
            )
            return None
        if not pd.api.types.is_numeric_dtype(
            ...
            print(
                f"PlotGenerator Error: Las columnas '{x_column}' o '{y_column}' no son numéricas para dispersión."
            )
            return None
    ...
    def generate_boxplot(
        ...
        if df is None or df.empty or column not in df.columns:
            print(
                f"PlotGenerator Error: Columna '{column}' no encontrada o DataFrame vacío para boxplot."
            )
            return None
        if not pd.api.types.is_numeric_dtype(df[column]):
            print(
                f"PlotGenerator Error: La columna '{column}' no es numérica para boxplot."
            )
            return None
        if by_column and by_column not in df.columns:
            print(
                f"PlotGenerator Error: Columna de agrupación '{by_column}' no encontrada para boxplot."
            )
            return None
    ...
    def generate_countplot(
        ...
        if df is None or df.empty or column not in df.columns:
            print(
                f"PlotGenerator Error: Columna '{column}' no encontrada o DataFrame vacío para countplot."
            )
            return None
